...
```{r}
correlation <- cor(na.omit(cancer2[, -c(9, 27)])) #Remove factor variables
corrplot(correlation, method = "circle", tl.cex = .5, number.cex = .4, type = "lower")
head(cancer2[, c(9, 27)]) # proof of removed vars
```
...
```{r}
model5 <- loess(data = train, target_deathrate ~ incidencerate + pctblack + pctnohs18_24)
summary(model5)
```
...
x <- subset(train, select = -c(pctemployed16_over))
formula <- paste("target_deathrate", "~", paste(names(x), collapse = " + "))
model6 <- ranger(formula, data = x, num.trees = 500, respect.unordered.factors = "order", importance = )
print(model6)
ranger::importance(model6)
?ranger
```
